...
{
  std::cout << "\n"
          << "========== CALIBRATION CUTS DEBUG ==========\n"
          << "Events                    : " << n_events << "\n"
          << "No PTD                    : " << n_no_ptd << "\n"
          << "No pCD                    : " << n_no_pcd << "\n"
          << "Particles                 : " << n_particles << "\n"
          << "No source vertex          : " << n_no_source_vertex << "\n"
          << "No single calo            : " << n_no_calo << "\n"
          << "Kinks                     : " << n_kinks << "\n"
          << "Low charge                : " << n_low_charge << "\n"
          << "Events passed cuts        : " << n_events_passed << "\n"
          << "ROOT Fill()               : " << n_filled << "\n"
          << "============================================\n";
  save_file_->cd();
...
if (!event.has("PTD"))
{
  ...
  if (n_no_ptd <= 10)
    std::cout << "DEBUG: event " << n_events << " -> NO PTD\n";
...
if (!event.has("pCD"))
{
  ...
  if (n_no_pcd <= 20)
    std::cout << "DEBUG: event " << n_events << " -> NO pCD\n";
  return dpp::base_module::PROCESS_SUCCESS;
}
  // Retrieve data banks
...
n_particles += PTD.particles().size();

if (n_events < 20)
{
  std::cout << "DEBUG: event " << n_events
            << " particles = " << PTD.particles().size()
            << " pCD hits = " << pCD.calorimeter_hits().size()
            << "\n";
}
...
n_no_source_vertex++;
  ...
  if (n_no_source_vertex <= 20)
    std::cout << "DEBUG: particle rejected -> NO SOURCE VERTEX\n"; 
     continue;
    }
  ...
  if (n_no_calo <= 20)
    std::cout << "DEBUG: particle rejected -> NO SINGLE CALO HIT\n";
...
int n_kink =
    particle->get_trajectory_handle()->get_pattern().number_of_kinks();
...
if(n_kink != 0)
{
  ...
  if (n_kinks <= 20)
    std::cout << "DEBUG: particle rejected -> KINKS = "
              << n_kink << "\n";
  ...
  if (n_low_charge <= 20)
    std::cout << "DEBUG: particle rejected -> LOW CHARGE = "
              << charge << " threshold = "
              << charge_threshold_ << "\n"; 
     continue;
    }
...
datatools::handle<snemo::datamodel::vertex>
calibration_cuts_module::vertex_close_to_a_calib_source(
    const datatools::handle<snemo::datamodel::particle_track> & particle)
{
    static int debug_count = 0;

    const auto & vertices = particle->get_vertices();
    ...
    for (const datatools::handle<snemo::datamodel::vertex> & vertex : vertices)
    {
        if (!vertex.has_data())
            continue;

        bool on_source_plane = vertex->is_on_reference_source_plane();
        ...
        if (debug_count < 50)
        {
            std::cout
                ...
                << "Y=" << ver_y
                << " Z=" << ver_z
                << " on_source_plane=" << on_source_plane
                << "\n";
        }
        ...
        for (int i = 0; i < calib_source_rows_; i++)
        {
            for (int j = 0; j < calib_source_columns_; j++)
            {
                ...
                double ellipse_distance =
                    (dy * dy) /
                    (source_cut_ellipse_Y_ * source_cut_ellipse_Y_) +

                    (dz * dz) /
                    (source_cut_ellipse_Z_ * source_cut_ellipse_Z_);

                if (debug_count < 50)
                {
                    std::cout
                        << "  source[" << i << "][" << j << "] "
                        << "Y=" << calib_source_Y_[i][j]
                        << " Z=" << calib_source_Z_[i][j]
    ...
    if (debug_count < 50)
        std::cout << "NO MATCHING SOURCE VERTEX\n";
